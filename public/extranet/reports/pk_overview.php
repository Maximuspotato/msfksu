...
$result = execute_request($c,$query);
...
<head>
<link rel="stylesheet" id="stylesheet" href="../CSS/reports.css" type="text/css" />
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title><?php echo $generalparams['title'];?></title>

</head>

<body>

<h2 class="h2_title"><?php echo $generalparams['title'];?></h2>
...
<?php
	...
	foreach($result_client as $one_client){
		echo '<option '.($one_client['PCT_NOM_LIV'] == $_REQUEST['client']?' selected="selected" ':'').'>'.$one_client['PCT_NOM_LIV'].'</option>';	
	}
?></select><br><br>
    ...
    <select name="country">
    	<option></option>
		<option value="KE" <?php if ($_REQUEST['country'] == "KE") echo "selected=selected"; ?>>Kenya</option>
		<option value="RG" <?php if ($_REQUEST['country'] == "RG") echo "selected=selected"; ?>>Region</option>
	</select><br><br>
...
<script>
	...
	for (var i = 1; i < trows.length; i++) {
		totWeight += parseFloat(trows[i].cells[3].innerHTML.replace(',',''));
		totVolume += parseFloat(trows[i].cells[4].innerHTML.replace(',','')/1000);
		totParcel += parseInt(trows[i].cells[5].innerHTML.replace(',',''));
		if (trows[i].cells[6].innerHTML == '') {
			trows[i].cells[6].innerHTML = '0'
		}
		totValue += parseFloat(trows[i].cells[6].innerHTML.replace(',',''));
	}
	if (totWeight) {
